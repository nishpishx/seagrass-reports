...
export default function SiteSummary({
  ...
  const allPlanned = site.sectors.every((s) => s.status === 'planned');
  const unitLabel = site.plantType === 'shoots' ? 'Shoots' : 'Seeds';

  return (
    <>
      <StatGrid
        stats={[
          { value: totalSeeds.toLocaleString(), label: allPlanned ? `${unitLabel} Planned` : `Total ${unitLabel}` },
                ...
                >
                  ...
                  <div className={styles.sectorInfo}>
                    ...
                    <div className={styles.sectorMeta}>
                      {data
                        ? sec.status === 'planned'
                          ? `${data.totalSeeds.toLocaleString()} ${site.plantType} planned`
                          : `${data.totalSeeds.toLocaleString()} ${site.plantType} · ${data.reportData.missions.length} missions`
                        : 'Loading…'}
                    </div>
                  </div>
      ...
      <SummaryCard
        ...
        text={`${site.name} (${site.region}) has ${totalSeeds.toLocaleString()} ${site.plantType} ${allPlanned ? 'planned' : 'planted'} across ${site.sectors.length} sectors. Select a sector to view its detailed report.`}
      />
    </>
  );
}
